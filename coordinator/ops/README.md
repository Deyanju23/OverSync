...
Local reference stack for the OverSync coordinator. **Not required in production** — Render deployments can point any external Prometheus at the `/metrics` endpoint.
...
# 6. Open Grafana  (admin / oversync)
open http://localhost:3001
```

The **OverSync Coordinator** dashboard is pre-loaded in Grafana under the *OverSync* folder.
...
- alert: CoordinatorDown
  expr: up{job="oversync-coordinator"} == 0
  for: 2m
  labels:
    severity: critical
  annotations:
